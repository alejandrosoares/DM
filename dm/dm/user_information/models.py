...
class UseOfCategories(models.Model):
    ...
    quantity = models.PositiveIntegerField(
        verbose_name="Cantidad de consultas", default=0)
...
class SearchWords(models.Model):
    ...
    quantity = models.PositiveIntegerField(
        verbose_name="Contador de cantidades de consultas", default=0)
...
class ProductsOfInterest(models.Model):
    ...
    idProduct = models.PositiveIntegerField(
        verbose_name="Id de producto de interes")
    quantity = models.PositiveIntegerField(
        verbose_name="Cantidad de visualizaciones")
    date = models.DateField(verbose_name="Fecha de registro",
...
class DateOfVisit(models.Model):
    ...
    date = models.DateField(verbose_name="Fecha visita",
                            auto_now_add=True, auto_now=False)
    lastPage = models.URLField(
        verbose_name="Pagina de la que proviene", null=True)
    browser = models.CharField(
        verbose_name="Navegador", max_length=20, null=True)
    versionBrowser = models.CharField(
        verbose_name="Version del Navegador", max_length=5, null=True)
...
def PostSaveProducto(sender, instance, created, **kwargs):
    if created:
        if instance.brand:
            ...
            name = instance.name
        SearchWords.objects.create(
            string=name, typeString='p', idObject=instance.id, img=instance.img, priority=False)
